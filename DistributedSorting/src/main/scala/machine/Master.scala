...
object MasterApp extends LazyLogging with Hyperparams {
...
class MasterServiceImpl(numWorkers: Int) extends MasterServiceGrpc.MasterService with LazyLogging {
  ...
  private var sortCompletedWorkers = Set.empty[Int]
  ...
  def sortComplete(request: SortCompleteRequest): Future[SortCompleteReply] = {
    sortCompletedWorkers += request.workerId
    logger.info(s"Sort completed by Worker ID: ${request.workerId}")

    if (sortCompletedWorkers.size == numWorkers) {
      logger.info("All workers completed Sorting.")
      workers.keys.foreach { workerId =>
        logger.info(s"Notifying Worker ID: $workerId to start sampling.")
      }
    }
    Future.successful(SortCompleteReply(success = true))
  }
  def pickBoundariesComplete(request: GetDataRequest): Future[GetDataReply] = {
    ...
    Future.successful(GetDataReply(partitionBoundaries = partitionBoundaries))
  }
  def sortStart(request: SortRequest): Future[SortReply] = {
    logger.info("Sort started for all workers")
    Future.successful(SortReply(success = true))
  }
  ...
  def merge(request: SortCompleteRequest): Future[SortCompleteReply] = {
